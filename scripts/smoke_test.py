...
def main() -> None:
    ...
    print(f"  Test samples  : {len(test_ds_raw)}")
    print(f"  Load time     : {time.perf_counter() - t0:.1f}s")

    # ── 4. Poisoning ────────────────────────────────────────────
    print("\nApplying BadNets poisoning ...")
    poisoned_ds = PoisonedDataset(train_ds_raw, cfg.poison, rng=np.random.default_rng(cfg.train.seed))
    triggered_test = build_triggered_test_set(test_ds_raw, cfg.poison)
